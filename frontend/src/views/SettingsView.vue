...
function addVoice() {
  voices.value.push({ reference_id: '', name: '', bound_speaker: null, pool_order: voices.value.length })
}
...
<template>
  <section class="page settings-page">
    ...
    <section class="panel voices-card">
      <div class="panel-heading"><div><h2>Fish 音色池</h2><span class="muted">固定角色名可绑定专属音色，其余角色按顺序稳定分配</span></div><button class="button secondary" @click="addVoice"><Plus :size="15" />添加音色</button></div>
      <div class="voice-table"><div class="voice-row voice-head"><span>显示名称</span><span>Fish Reference ID</span><span>固定角色（可选）</span><span></span></div><div v-for="(voice, index) in voices" :key="voice.id || index" class="voice-row"><input v-model="voice.name" placeholder="旁白" /><input v-model="voice.reference_id" placeholder="reference id" /><input v-model="voice.bound_speaker" placeholder="NARRATOR" /><button class="icon-button" title="移除音色" @click="removeVoice(index)"><Trash2 :size="15" /></button></div><p v-if="!voices.length" class="empty-voices">还没有配置音色，开始渲染前至少添加一个。</p></div>
    </section>
    <p v-if="error" class="form-error settings-error">{{ error }}</p>
    <div class="settings-actions"><span v-if="saved" class="success-text">设置已保存</span><button class="button primary" @click="save"><Save :size="17" />保存设置与音色</button></div>
  </section>
</template>
