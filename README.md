...
> The default application is now the Fish Audio whole-book workflow, with a FastAPI/SQLite backend and Vue 3 frontend. New projects render after each reviewed batch of three chapters by default, or can wait for all reviews. The legacy Qwen TTS, voice design, LoRA, and dataset code remains in the repository but is not mounted by the new entry point.

For development, install `backend/requirements.txt` and the `frontend` npm dependencies, then run `python -m backend.alexandria.main` and `npm --prefix frontend run dev`. A production frontend build is served by FastAPI at `http://127.0.0.1:4200`.

The CLI uses the same default: `python tools/render_book.py --epub book.epub --render-start after-review-batch --release-batch-size 3`. Use `--render-start after-all-reviews` for the previous wait-for-the-whole-book behavior.
